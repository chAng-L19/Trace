...
from ..core import Evidence, Event, Goal, Run, RUN_STATUSES, TerminalDecision
...
ALLOWED_RUN_TRANSITIONS: Final = {
    ...
    "running": frozenset({"running", "waiting_worker", "paused_budget", "cancelling", "cancelled", "completed", "failed"}),
    "waiting_worker": frozenset({"waiting_worker", "running", "paused_budget", "cancelling", "cancelled", "completed", "failed"}),
    "paused_budget": frozenset({"paused_budget", "running", "waiting_worker", "cancelling", "cancelled", "completed", "failed"}),
...
def validate_run_transition(previous: str, current: str) -> None:
    if previous not in CANONICAL_RUN_STATUSES or current not in CANONICAL_RUN_STATUSES:
        raise ValueError(f"run_status_invalid:{previous if previous not in CANONICAL_RUN_STATUSES else current}")
