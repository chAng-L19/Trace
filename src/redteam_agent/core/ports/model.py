...
class Event:
    ...
    def to_dict(self) -> dict[str, Any]:
        return versioned_payload(self.KIND, {"run_id": self.run_id, "event_type": self.event_type, "payload": dict(self.payload), "sequence": self.sequence, "created_at": self.created_at, "metadata": dict(self.metadata)})

    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> "Event":
        contract_version(payload, kind=cls.KIND)
        return cls(required_text(payload.get("run_id"), "event_run_id"), required_text(payload.get("event_type") or payload.get("type"), "event_type"), json_mapping(payload.get("payload"), field="event.payload"), bounded_int(payload.get("sequence", payload.get("event_id", 0)), default=0, minimum=0, maximum=2**63 - 1, field="event_sequence"), optional_text(payload.get("created_at")), json_mapping(payload.get("metadata"), field="event.metadata"))
